...
printf ( "\n => " );j = getREindex ( temp );
temp[j] = '\0';
printf ( temp2 , "%s%s%s" , temp , productions[i] , (temp + j + 2) );
printf ( "%s" , temp2 );
strcpy ( temp , temp2 );
}
}
...
void yylex() /*calls lex and takes each character as input and feeds ALPHABET to check for the
structure*/
{
...
void yyerror() /*Function to alert user of invalid regular expressions*/
{
